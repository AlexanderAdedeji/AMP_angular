...
Cancel(){
  ...
  if(this.isSuperUser)this.router.navigate(['AMP/agentdetails/agentadmins'])
  else this.router.navigate(['AMP/supervisors/allSupervisors'])
}
  ...
  GetUserTypes() {
    this.SupervisorUserType= this.storageService.AllUserTypes.filter(UserType=>(UserType.name == "AGENT_SUPERVISOR"));
    ...
    createANewAgentUser(value){
      ...
      value.token = 'Token ' + JSON.parse(sessionStorage.getItem('user')).token;
      value.user_type_id = this.SupervisorUserType[0].id
    ...
    //   const newValue = this.encodeAESService.encryptData(value)
    //   this.loaderService.show();
    //   this.authService.registerAgentUser(newValue, this.storageService.token)
    //     .subscribe((res: any) => {
